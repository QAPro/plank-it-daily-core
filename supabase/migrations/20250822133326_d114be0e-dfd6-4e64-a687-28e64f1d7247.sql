...
-- 1) Helper function to avoid recursive policies on challenge_participants
...
-- 2) Update existing SELECT policy to use the helper function (only if the policy exists)
do $policy$
declare
  has_policy boolean;
begin
  select exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'Users can view participants of joined challenges'
  ) into has_policy;
...
-- 3) Enforce fixed search_path on all public functions missing it (idempotent)
do $$
declare 
  r record;
begin
...
-- 4) Lock down materialized views so they are not directly accessible via the API
do $$
begin
