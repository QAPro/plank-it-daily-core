-- Phase 3: Event-Driven Notifications (triggers + schedules)
-- Ensure search_path and security definer for all functions

-- Helper: build the notification-events URL once
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'notify_via_edge_function'
  ) THEN
...
DROP TRIGGER IF EXISTS trg_after_insert_user_sessions ON public.user_sessions;
CREATE TRIGGER trg_after_insert_user_sessions
AFTER INSERT ON public.user_sessions
FOR EACH ROW EXECUTE FUNCTION public.trg_notify_session_completed();

-- 4) Cron schedules for daily reminders, streak risk, weekly summary
-- Guard schedules to avoid duplicates
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- Daily workout reminders every 5 minutes
    PERFORM cron.schedule(
      job_name => 'daily-workout-reminders',
      schedule => '*/5 * * * *',
      command => $$
    ...
    -- Streak risk alert daily at 18:00 UTC
    PERFORM cron.schedule(
      job_name => 'streak-risk-alerts',
      schedule => '0 18 * * *',
      command => $$
        select net.http_post(
          url:='https://kgwmplptoctmoaefnpfg.supabase.co/functions/v1/notification-events',
    ...
    -- Weekly summary on Sundays at 18:00 UTC
    PERFORM cron.schedule(
      job_name => 'weekly-progress-summary',
      schedule => '0 18 * * 0',
      command => $$
        select net.http_post(
          url:='https://kgwmplptoctmoaefnpfg.supabase.co/functions/v1/notification-events',
  ...
  ELSE
    RAISE NOTICE 'pg_cron extension not installed; skipping cron schedules';
  END IF;
END $$;