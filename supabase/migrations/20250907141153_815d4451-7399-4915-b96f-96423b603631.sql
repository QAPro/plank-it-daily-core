...
CREATE POLICY IF NOT EXISTS "Users can manage own reminder slots"
...
-- Admins can view all
CREATE POLICY IF NOT EXISTS "Admins can view all reminder slots"
ON public.user_reminder_slots
FOR SELECT
...
-- Updated at trigger
CREATE TRIGGER IF NOT EXISTS set_updated_at_user_reminder_slots
...
CREATE POLICY IF NOT EXISTS "Admins can manage notification message variants"
...
CREATE TRIGGER IF NOT EXISTS set_updated_at_notification_message_variants
...
CREATE POLICY IF NOT EXISTS "Admins can view all notification variant assignments"
ON public.user_notification_variant_assignments
FOR SELECT
USING (public.is_admin(auth.uid()));

CREATE POLICY IF NOT EXISTS "Users can view own notification variant assignments"
ON public.user_notification_variant_assignments
FOR SELECT
USING (auth.uid() = user_id);

-- Allow system (service role) to insert
CREATE POLICY IF NOT EXISTS "System can insert notification variant assignments"
ON public.user_notification_variant_assignments
FOR INSERT
WITH CHECK (true);
