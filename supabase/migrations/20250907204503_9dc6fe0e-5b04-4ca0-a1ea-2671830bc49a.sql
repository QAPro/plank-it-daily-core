-- Phase 2 (retry): Social Investment Amplification - use unique names and IF NOT EXISTS to avoid conflicts

-- 1) Victory Partnerships
CREATE TABLE IF NOT EXISTS public.victory_partnerships (
...
CREATE TABLE IF NOT EXISTS public.victory_partner_checkins (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.user_success_stories (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  ...
  transformation_data JSONB DEFAULT '{}'::jsonb,
...
CREATE TABLE IF NOT EXISTS public.success_story_reactions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.success_story_comments (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  story_id UUID NOT NULL REFERENCES public.user_success_stories(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- 3) Community Success Challenges
CREATE TABLE IF NOT EXISTS public.community_success_challenges (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Use a unique table name to avoid conflicts with any existing challenge_participants
...
ON public.success_story_comments FOR SELECT 
USING (EXISTS (
  SELECT 1 FROM public.user_success_stories uss 
...
ON public.success_story_comments FOR INSERT 
WITH CHECK (auth.uid() = user_id);

-- Challenges
CREATE POLICY "Users can view public challenges and own challenges" 
ON public.community_success_challenges FOR SELECT 
USING (is_public = true OR auth.uid() = creator_id);

CREATE POLICY "Users can create challenges" 
ON public.community_success_challenges FOR INSERT 
WITH CHECK (auth.uid() = creator_id);

CREATE POLICY "Creators can update own challenges" 
ON public.community_success_challenges FOR UPDATE 
...
USING (EXISTS (
  ...
  WHERE csc.id = community_challenge_participants.challenge_id 
