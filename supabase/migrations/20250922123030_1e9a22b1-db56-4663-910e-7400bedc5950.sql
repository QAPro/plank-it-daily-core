...
-- Create trigger to log sensitive user data access
...
AS $$
BEGIN
  -- Log when admins access user data
  IF EXISTS (
    SELECT 1 FROM user_roles 
    WHERE user_id = auth.uid() 
    AND role = 'admin'::app_role
    ...
    INSERT INTO user_data_access_log (
      ...
      NEW.id,
      'admin_view',
      ARRAY['username', 'subscription_tier', 'current_level', 'total_xp'],
      'Administrative access via RLS policy'
    );
  END IF;
...
CREATE TRIGGER log_admin_user_access
  AFTER SELECT ON public.users
  FOR EACH ROW
  EXECUTE FUNCTION log_user_data_access();