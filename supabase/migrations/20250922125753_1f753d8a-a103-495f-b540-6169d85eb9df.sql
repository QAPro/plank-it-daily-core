...
-- 2. Users can only update their own profile (but not email directly for security)
...
WITH CHECK (
  auth.uid() = id 
  AND email = OLD.email  -- Prevent direct email changes via this policy
);
...
-- 4. Admins can view user data (but through secure functions only)
...
-- Audit logging will be handled through secure functions instead
